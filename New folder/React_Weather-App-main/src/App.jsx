    ...
    if (cityName) {
      fetch(`https://api.openweathermap.org/data/2.5/weather?units=metric&q=${cityName}&appid=23452a215e99c24e452175b6ff4e8081`)
        ...
        .then((data) => {
          ...
          const key = `${cityName}_${Date.now()}`;

          localStorage.setItem(key, [cityName, `${Math.round(data.main.temp)}`, `${date}`])
    ...
    <div className="App">
      <div className="main">
        <div className='childDiv'>
          ...
          <Link to={'/history'}><button id='searchbtn' style={{ width: '90px' }}>History</button></Link>
        </div>
